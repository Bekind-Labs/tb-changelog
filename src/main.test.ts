...
import type { loadCacheIfExists } from "./utils/cache";
...
describe("main", () => {
  ...
  it("displays help information when requested", async () => {
    await main(["--help"]);
  ...
  describe("terminates with error when validation fails", () => {
    it("exits with code 1 when TB_API_KEY is not provided", async () => {
      await main([]);
    ...
    ])("exits with code 1 ($expected)", async ({ args, expected }) => {
  ...
  it("executes changelog generation when inputs are valid", async () => {
    vi.stubEnv("TB_API_KEY", "test-key");
  ...
  describe("outputs markdown content based on configuration", () => {
    ...
    it("includes signature by default", async () => {
      await main(["--from", "v1.0.0", "--tb-project-id", "123456"]);

      const actualOutput = vi.mocked(console.log).mock.lastCall![0];
      expect(actualOutput).toContain("*Generated by [tb-changelog](https://github.com/Bekind-Labs/tb-changelog)*");
    });

    it("excludes signature when --no-signature is specified", async () => {
      await main(["--from", "v1.0.0", "--tb-project-id", "123456", "--no-signature"]);

      const actualOutput = vi.mocked(console.log).mock.lastCall![0];
      expect(actualOutput).not.toContain("*Generated by [tb-changelog](https://github.com/Bekind-Labs/tb-changelog)*");
    });

    describe("format: slack-payload", () => {
      it("outputs valid JSON when format is slack-payload", async () => {
        await main(["--from", "v1.0.0", "--tb-project-id", "123456", "--format", "slack-payload"]);

        const actualOutput = vi.mocked(console.log).mock.lastCall![0];

        expect(() => JSON.parse(actualOutput)).not.toThrow();

        const payload = JSON.parse(actualOutput);
        expect(payload).toHaveProperty("channel", "<your-channel>");
        expect(payload).toHaveProperty("attachments");
        expect(payload.attachments[0]).toHaveProperty("color", "<your-color>");
        expect(payload.attachments[0]).toHaveProperty("blocks");

        expect(actualOutput).toContain("_Generated by <https://github.com/Bekind-Labs/tb-changelog|tb-changelog>_");
      });

      it("excludes signature in slack-payload when --no-signature is specified", async () => {
        await main(["--from", "v1.0.0", "--tb-project-id", "123456", "--format", "slack-payload", "--no-signature"]);

        const actualOutput = vi.mocked(console.log).mock.lastCall![0];
        expect(actualOutput).not.toContain("_Generated by <https://github.com/Bekind-Labs/tb-changelog|tb-changelog>_");
      });
    });
  });
  ...
  describe("cache functionality", () => {
    ...
    it("does not create cache file when --use-cache is not specified", async () => {
      await main(["--from", "v1.0.0", "--tb-project-id", "123456"]);
    ...
    it("creates cache file and retrieve data on first run with --use-cache", async () => {
      vi.mocked(readFile).mockRejectedValue(new Error("ENOENT"));

      await main(["--from", "v1.0.0", "--tb-project-id", "123456", "--use-cache"]);

      expect(readFile).toHaveBeenCalledWith("./.tb-changelog-cache", "utf-8");
      expect(writeFile).toHaveBeenCalledWith("./.tb-changelog-cache", expect.stringContaining('"key":'), "utf-8");

      expect(getCommits).toHaveBeenCalledTimes(1);
      expect(fetchStoriesFromApi).toHaveBeenCalledTimes(1);
    });

    it("uses cached data and don't retrieve data on second run with same parameters", async () => {
      const expectedCacheKey = "123456::v1.0.0--HEAD";
      const cacheData = {
      ...
      } satisfies NonNullable<Awaited<ReturnType<typeof loadCacheIfExists>>>;

      vi.mocked(readFile).mockResolvedValue(JSON.stringify(cacheData));

      await main(["--from", "v1.0.0", "--tb-project-id", "123456", "--use-cache"]);

      expect(readFile).toHaveBeenCalledWith("./.tb-changelog-cache", "utf-8");
      expect(getCommits).not.toHaveBeenCalled();
      expect(fetchStoriesFromApi).not.toHaveBeenCalled();
      expect(consola.success).toHaveBeenCalledWith(expect.stringContaining("Found and matched cache"));
    });

    it("invalidates cache when parameters change", async () => {
      const cacheDataWithWrongKey = {
        key: "123456::v1.0.0--HEAD",
        timestamp: "2024-01-01T00:00:00Z",
        data: {
          acceptedStories: [],
          needsAttentionStories: [],
      ...
      vi.mocked(readFile).mockResolvedValue(JSON.stringify(cacheDataWithWrongKey));

      await main(["--from", "v2.0.0", "--tb-project-id", "123456", "--use-cache"]);
