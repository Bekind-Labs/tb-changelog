...
describe("main", () => {
  ...
  describe("outputs markdown content based on configuration", () => {
    ...
    it("includes signature by default", async () => {
      ...
      const actualOutput = vi.mocked(console.log).mock.lastCall![0];
      expect(actualOutput).toContain("*Generated by [tb-changelog](https://github.com/yukkobay/tb-changelog)*");
    });
    ...
    it("excludes signature when --no-signature is specified", async () => {
      ...
      const actualOutput = vi.mocked(console.log).mock.lastCall![0];
      expect(actualOutput).not.toContain("*Generated by [tb-changelog](https://github.com/yukkobay/tb-changelog)*");
    });
  });
});
